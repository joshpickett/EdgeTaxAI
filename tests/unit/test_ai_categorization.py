...
def test_add_expense_with_ai_categorization(client, mocker):
    ...
    mocker.patch(
        "api.routes.expense_routes.categorize_expense", return_value="AI-Categorized"
    )
    response = client.post(
        "/add", json={"user_id": 1, "description": "Business Lunch", "amount": 35.0}
    )
...
def test_add_expense_empty_description(client, mocker):
    ...
    mocker.patch(
        "api.routes.expense_routes.categorize_expense", return_value="Uncategorized"
    )
    response = client.post(
        "/add", json={"user_id": 1, "description": "", "amount": 15.0}
    )
...
def test_add_expense_default_category(client, mocker):
    ...
    response = client.post(
        "/add", json={"user_id": 1, "description": "Miscellaneous", "amount": 10.0}
    )
