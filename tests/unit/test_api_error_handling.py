...
def test_plaid_public_token_exchange_api_failure(mock_exchange, client):
    ...
    response = client.post("/plaid/exchange-token", json={"user_id": 1, "public_token": "mock-token"})
...
def test_plaid_transactions_api_failure(mock_transactions_get, client):
    ...
    response = client.get("/plaid/transactions?user_id=1&start_date=2024-12-01&end_date=2024-12-20")
