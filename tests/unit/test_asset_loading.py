...
def test_image_formats():
    ...
    for svg in svg_files:
        with open(svg, 'r') as f:
            content = f.read()
            assert content.startswith('<?xml') or content.startswith('<svg'), f"Invalid SVG format: {svg}"
...
def test_favicon_sizes():
    ...
    for size in required_sizes:
        assert (favicon_dir / f'favicon-{size}.png').exists(), f"Missing favicon size: {size}"
