...
def test_color_consistency():
    ...
    with open('src/styles/tokens.js', 'r') as file:
        ...
        primary_color = re.search(r'main: [\'"]#([0-9A-Fa-f]{6})[\'"]', content).group(1)
        ...
        for javascript_file in components_directory.rglob('*.js'):
            with open(javascript_file, 'r') as component_file:
                ...
                assert primary_color.lower() in component_content.lower(), f"Inconsistent color usage in {javascript_file}"
...
def test_typography_consistency():
    ...
    with open('src/styles/tokens.js', 'r') as file:
        ...
        for javascript_file in components_directory.rglob('*.js'):
            with open(javascript_file, 'r') as component_file:
                component_content = component_file.read()
                assert font_family in component_content, f"Inconsistent font usage in {javascript_file}"
        ...
        for javascript_file in components_directory.rglob('*.js'):
            with open(javascript_file, 'r') as component_file:
                ...
                for spacing_value in re.findall(r'\d+', spacing_values):
                    assert spacing_value in component_content, f"Inconsistent spacing in {javascript_file}"
    ...
    with open('src/styles/tokens.js', 'r') as f:
        ...
        for js_file in components_dir.rglob('*.js'):
            with open(js_file, 'r') as component_file:
                component_content = component_file.read()
                assert 'darkMode' in component_content, f"Missing dark mode support in {js_file}"
