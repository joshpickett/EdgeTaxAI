...
class ProfilePage:
    ...
    def profile_page(self):
        ...
        try:
            response = requests.get(
                f"{API_BASE_URL}/auth/profile", params={"user_id": self.user_id}
            )
            ...
            if not self.validate_email(email):
                ...
                try:
                    ...
                    update_response = requests.put(
                        f"{API_BASE_URL}/auth/profile", json=payload
                    )
    ...
    def render_tax_documents(self):
        ...
        # Document categories
        doc_tabs = streamlit.tabs(
            ["Generated Documents", "Upload Documents", "Archive"]
        )
    ...
    def show_generated_documents(self):
        ...
        selected_year = streamlit.selectbox(
            "Select Year", range(datetime.now().year, 2020, -1)
        )
    ...
    def upload_tax_documents(self):
        ...
        doc_type = streamlit.selectbox(
            "Document Type", ["W-9", "1099-NEC", "Receipts", "Other"]
        )

        # File uploader
        uploaded_file = streamlit.file_uploader(
            "Choose a file", type=["pdf", "jpg", "png"], key="tax_doc_upload"
        )
    ...
    def show_archived_documents(self):
        ...
        year_filter = streamlit.multiselect(
            "Filter by Year", range(datetime.now().year, 2020, -1)
        )
    ...
    def render_security_settings(self):
        ...
        enable_two_factor_authentication = streamlit.toggle(
            "Enable Two-Factor Authentication", value=False
        )
            ...
            if connected_platforms:
                ...
                for platform in connected_platforms:
                    with streamlit.expander(f"{platform['platform'].title()} Settings"):
                        # Platform status
                        status = validate_platform_connection(
                            self.user_id, platform["platform"]
                        )
                        if status.connected:
                            streamlit.success("Connected")
                            if status.last_sync:
                                streamlit.write(
                                    f"Last synced: {status.last_sync.strftime('%Y-%m-%d %H:%M:%S')}"
                                )
                        else:
                        ...
                        # Sync settings
                        auto_sync = streamlit.checkbox(
                            "Enable auto-sync", key=f"auto_sync_{platform['platform']}"
                        )
