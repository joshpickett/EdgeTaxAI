...
def test_verify_otp_success(mock_streamlit, mock_requests):
    ...
    with patch.dict(streamlit.session_state, {'is_otp_sent': True, 'identifier': 'test@example.com'}):
        account_creation_page('http://test-api')
        
        mock_streamlit.text_input.return_value = "123456"
        mock_streamlit.button.return_value = True
        
        mock_streamlit.success.assert_called_with("Account created successfully! Please log in.")
