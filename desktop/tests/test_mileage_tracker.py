...
def test_track_new_trip_success(mock_mileage_service):
    with patch("streamlit.text_input") as mock_input, patch(
    ...
    ) as mock_date, patch("streamlit.button") as mock_button, patch(
        "streamlit.success"
    ) as mock_success:
        ...
        mock_mileage_service.calculateMileage.return_value = {
            "status_code": 200,
            "data": {"distance": 10.5, "tax_deduction": 5.25},
        }
...
def test_track_new_trip_validation_error(mock_mileage_service):
    with patch("streamlit.text_input") as mock_input, patch(
        "streamlit.button"
    ) as mock_button, patch("streamlit.error") as mock_error:
...
def test_recurring_trip_functionality(mock_mileage_service):
    with patch("streamlit.text_input") as mock_input, patch(
        "streamlit.date_input"
    ) as mock_date, patch("streamlit.checkbox") as mock_checkbox, patch(
        "streamlit.selectbox"
