...
def mock_streamlit():
    with patch("streamlit.title") as mock_title, patch(
        "streamlit.image"
    ) as mock_image, patch("streamlit.markdown") as mock_markdown, patch(
...
def test_tax_deductions_page_initialization(mock_streamlit):
    ...
    mock_streamlit["markdown"].assert_called_with(
        "#### Optimize your expenses with AI-detected deductions."
    )
...
def test_file_upload_processing(mock_requests):
    ...
    with patch("streamlit.file_uploader") as mock_uploader, patch(
        "streamlit.dataframe"
    ) as mock_dataframe:
...
def test_manual_input_processing(mock_requests):
    ...
    with patch("streamlit.text_input") as mock_text, patch(
        "streamlit.number_input"
    ) as mock_number, patch("streamlit.dataframe") as mock_dataframe:
...
def test_api_error_handling(mock_requests):
    ...
    ) as mock_text, patch("streamlit.number_input") as mock_number:
        mock_text.return_value = "Test expense"
        mock_number.return_value = 100.00

        tax_deductions_page()
        mock_error.assert_called_with(
            "An error occurred while fetching tax deductions: API Error"
        )
