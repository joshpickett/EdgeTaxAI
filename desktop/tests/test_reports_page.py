...
def mock_streamlit():
    with patch("streamlit.title") as mock_title, patch(
        "streamlit.markdown"
    ) as mock_markdown, patch("streamlit.session_state") as mock_session:
        mock_session.user_id = 123
        yield {"title": mock_title, "markdown": mock_markdown, "session": mock_session}
...
def test_reports_page_initialization(mock_streamlit):
    ...
    mock_streamlit["markdown"].assert_called_with(
        "#### View, Generate, and Manage Reports"
    )
...
def test_irs_report_generation_success(mock_requests):
    ...
    mock_response.json.return_value = {"pdf": b"mock_pdf_data", "csv": b"mock_csv_data"}
    mock_requests["get"].return_value = mock_response

    with patch("streamlit.button") as mock_button, patch(
        "streamlit.success"
    ) as mock_success, patch("streamlit.download_button") as mock_download:
...
def test_schedule_c_generation(mock_requests):
    with patch("streamlit.button") as mock_button, patch(
        "streamlit.markdown"
    ) as mock_markdown, patch("streamlit.success") as mock_success:
        mock_button.return_value = True
        reports_page()
        mock_success.assert_called_once_with(
            "IRS Schedule C Report generated successfully!"
        )
...
def test_custom_report_generation(mock_requests):
    ...
    with patch("streamlit.date_input") as mock_date, patch(
        "streamlit.text_input"
    ) as mock_text, patch("streamlit.button") as mock_button, patch(
        "streamlit.success"
    ) as mock_success:
...
def test_gig_platform_connection_display(mock_requests):
    ...
    mock_response.json.return_value = {
        "connected_accounts": [{"platform": "uber"}, {"platform": "lyft"}]
    }
    mock_requests["get"].return_value = mock_response
...
def test_fetch_platform_data(mock_requests):
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "data": [{"trip_id": 1, "amount": 50.0}, {"trip_id": 2, "amount": 75.0}]
    }
    mock_requests["get"].return_value = mock_response

    with patch("streamlit.selectbox") as mock_select, patch(
        "streamlit.button"
    ) as mock_button, patch("streamlit.success") as mock_success, patch(
        "streamlit.dataframe"
    ) as mock_dataframe:
