...
class TestTaxRoutes:
    def test_estimate_quarterly_tax_success(self, client, mock_calculator):
        ...
        response = client.post('/api/tax/estimate-quarterly', json={
            'user_id': 123,
            'year': 2023,
            'quarter': 1
        })
    ...
    def test_real_time_tax_savings_success(self, client, mock_calculator):
        ...
        response = client.post('/api/tax/savings', json={
            'amount': 1000.00,
            'user_id': 123
        })
    ...
    def test_ai_deduction_suggestions_success(self, client):
        response = client.post('/api/tax/deductions', json={
            'expenses': [
    ...
    def test_calculate_effective_rate_success(self, client):
        response = client.post('/calculate-effective-rate', json={
        ...
        assert response.status_code == 200
        data = response.json
        assert all(key in data for key in ['taxable_income', 'tax_bracket', 'tax_rate', 'estimated_tax'])

    def test_generate_tax_document_success(self, client):
        response = client.post('/api/tax/document', json={
            'user_id': 123,
            'year': 2023,
