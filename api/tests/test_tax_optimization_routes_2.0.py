...
class TestTaxOptimizationRoutes:
    def test_optimize_tax_strategy_success(self, client, mock_tax_service):
        mock_tax_service.return_value.calculate_tax_savings.return_value = Decimal(
            "1000.00"
        )

        response = client.post("/tax/optimize", json={"user_id": 123, "year": 2023})
    ...
    def test_analyze_deductions_success(self, client, mock_tax_service):
        ...
        response = client.post(
            "/analyze-deductions", json={"user_id": 123, "year": 2023}
        )
    ...
    def test_enhanced_analyze_success(self, client, mock_tax_service):
        ...
        response = client.post(
            ...
            json={
                "user_id": 123,
                "quarter": 1,
                "expense_data": {"category": "office_supplies", "amount": 500.00},
            },
        )
    ...
    def test_missing_required_fields(self, client):
        response = client.post(
            "/enhanced-analyze", json={"quarter": 1}  # Missing user_id
        )
    ...
    def test_invalid_quarter(self, client):
        response = client.post(
            "/enhanced-analyze",
            json={"user_id": 123, "quarter": 5, "expense_data": {}},  # Invalid quarter
        )
    ...
    def test_optimization_suggestions_quality(self, client, mock_tax_service):
        ...
        response = client.post(
            "/enhanced-analyze", json={"user_id": 123, "quarter": 1, "expense_data": {}}
        )
    ...
    def test_tax_savings_calculation_accuracy(self, client, mock_tax_service):
        ...
        for income, expected_savings in test_cases:
            mock_tax_service.return_value.calculate_tax_savings.return_value = Decimal(
                str(expected_savings)
            )

            response = client.post(
                "/tax/optimize", json={"user_id": 123, "year": 2023, "income": income}
            )
            ...
            assert (
                abs(
                    float(
                        response.json["optimization_suggestions"]["potential_savings"]
                    )
                    - expected_savings
                )
                < 0.01
    ...
    def test_deduction_validation(self, client):
        ...
        for deduction in invalid_deductions:
            response = client.post(
                "/analyze-deductions",
                json={"user_id": 123, "year": 2023, "deductions": [deduction]},
            )
    ...
    def test_optimization_strategy_prioritization(self, client, mock_tax_service):
        mock_strategies = {
            ...
            "suggestions": [
                ...
                {"category": "misc", "potential_savings": 100.00, "priority": "low"},
            ],
        }
    ...
    def test_optimization_with_historical_data(self, client, mock_tax_service):
        ...
        mock_tax_service.return_value.analyze_tax_context.return_value = {
            ...
            "historical_comparison": {"improvement": 0.05, "trending": "positive"},
        }

        response = client.post(
            "/tax/optimize",
            json={"user_id": 123, "year": 2023, "include_historical": True},
        )
    ...
    def test_category_specific_optimization(self, client, mock_tax_service):
        ...
        for category in categories:
            ...
            response = client.post(
                "/analyze-deductions",
                json={"user_id": 123, "year": 2023, "category": category},
            )
            ...
            assert (
                response.json["potential_deductions"]
                > response.json["current_deductions"]
            )
