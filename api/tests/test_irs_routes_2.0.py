...
class TestIRSRoutes:
    def test_generate_schedule_c_success(self, client, mock_database):
        ...
        response = client.post(
            "/generate-schedule-c", json={"user_id": 123, "year": 2023}
        )
    ...
    def test_generate_schedule_c_missing_user(self, client):
        response = client.post("/generate-schedule-c", json={"year": 2023})
    ...
    def test_quarterly_estimate_success(self, client, mock_database):
        ...
        response = client.post(
            "/quarterly-estimate", json={"user_id": 123, "quarter": 1, "year": 2023}
        )
    ...
    def test_quarterly_estimate_invalid_quarter(self, client):
        response = client.post(
            "/quarterly-estimate", json={"user_id": 123, "quarter": 5, "year": 2023}
        )
        assert response.status_code == 400
        assert "error" in response.json
    ...
    def test_calculate_deductions_success(self, client, mock_database):
        ...
        response = client.post(
            "/calculate-deductions", json={"user_id": 123, "year": 2023}
        )
    ...
    def test_tax_category_validation(self, client):
        response = client.post(
            "/validate-category",
            json={"category": "invalid_category", "amount": 100.00},
        )
        assert response.status_code == 400
        assert "error" in response.json
    ...
    def test_expense_categorization(self, client):
        response = client.post(
            "/categorize-expense",
            json={"description": "Office supplies from Staples", "amount": 150.00},
        )
        assert response.status_code == 200
    ...
    def test_estimated_payments_calculation(self, client):
        response = client.post(
            "/estimated-payments",
            json={"income": 50000.00, "expenses": 20000.00, "quarter": 1},
        )
        assert response.status_code == 200
    ...
    def test_deduction_limits_validation(self, client):
        response = client.post(
            "/validate-deduction",
            json={"category": "meals", "amount": 100000.00},  # Exceeds reasonable limit
        )
        assert response.status_code == 400
        assert "error" in response.json
