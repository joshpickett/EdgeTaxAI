...
async def test_process_platform_data_success(gig_processor, mock_uber_data):
    ...
    with patch("api.utils.gig_platform_processor.UberProcessor") as mock_uber:
        ...
        mock_uber.return_value.process_earnings.return_value = {"total_earnings": 25.50}
...
async def test_create_expense_entries(gig_processor):
    ...
    trips = [
        {"id": "trip123", "description": "Test trip", "expenses": 50.0, "type": "rides"}
    ]
...
def test_validate_processed_data(gig_processor):
    ...
    gig_processor._validate_processed_data(valid_data["trips"], valid_data["earnings"])

    # Test invalid data
    with pytest.raises(ValueError):
        gig_processor._validate_processed_data("invalid", valid_data["earnings"])
