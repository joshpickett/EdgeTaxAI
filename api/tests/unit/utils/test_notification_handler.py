...
def test_send_sms_success(notification_handler, mock_twilio):
    ...
    mock_twilio.return_value.messages.create.assert_called_once_with(
        body=message,
        from_=notification_handler.twilio_phone_number,
        to=phone_number
    )
...
def test_send_email_failure(notification_handler, mock_smtp):
    ...
    mock_smtp.return_value.__enter__.return_value.send_message.side_effect = Exception("Email failed")
...
def test_send_expense_alert_success(notification_handler):
    ...
    with patch.object(notification_handler, 'send_sms') as mock_sms, \
         patch.object(notification_handler, 'send_email') as mock_email:
