...
def biometric_auth(mock_database):
    with patch(
        "api.utils.biometric_auth.get_database_connection", return_value=mock_database
    ):
...
def test_register_biometric_success(biometric_auth, mock_database):
    ...
    result = biometric_auth.register_biometric(
        user_id=1, biometric_data="sample_biometric_data"
    )
...
def test_verify_biometric_success(biometric_auth, mock_database):
    ...
    mock_cursor.fetchone.return_value = ("stored_hash", os.urandom(32))  # Random salt
    mock_database.cursor.return_value = mock_cursor

    with patch.object(biometric_auth, "_hash_biometric", return_value="stored_hash"):
...
def test_verify_biometric_invalid_hash(biometric_auth, mock_database):
    ...
    mock_cursor = Mock()
    mock_cursor.fetchone.return_value = ("stored_hash", os.urandom(32))
    mock_database.cursor.return_value = mock_cursor

    with patch.object(biometric_auth, "_hash_biometric", return_value="different_hash"):
        result = biometric_auth.verify_biometric(
            user_id=1, biometric_data="sample_biometric_data"
        )
