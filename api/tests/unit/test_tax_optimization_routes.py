...
def test_optimize_tax_strategy(client):
    ...
    with patch(
        "api.routes.tax_optimization_routes.analyze_optimization_opportunities"
    ) as mock_analyze:
        ...
        response = client.post("/tax/optimize", json={"user_id": 1, "year": 2023})
...
def test_analyze_deductions(client):
    ...
    response = client.post("/analyze-deductions", json={"user_id": 1, "year": 2023})
...
def test_analyze_deductions_invalid_user(client):
    ...
    response = client.post("/analyze-deductions", json={"year": 2023})
...
def test_enhanced_analyze_invalid_data(client):
    ...
    response = client.post("/enhanced-analyze", json={"user_id": 1, "quarter": 1})
