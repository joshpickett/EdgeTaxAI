...
def test_add_expense_success(mock_expense_service, mock_sync_manager):
    # Arrange
    expense_data = {"description": "Test expense", "amount": 100.00, "category": "test"}
...
def test_edit_expense_success(mock_expense_service):
    # Arrange
    expense_id = 1
    updated_data = {"description": "Updated expense", "amount": 150.00}
    ...
    # Assert
    assert result is True
    mock_expense_service.update_expense.assert_called_once_with(
        expense_id, updated_data
    )
