...
def test_generate_tax_summary(client):
    ...
    with patch('api.routes.reports_routes.report_generator') as mock_generator:
        ...
        response = client.post('/reports/tax-summary', json={
            'user_id': 1,
            'year': 2023
        })
...
def test_generate_quarterly_summary(client):
    ...
    with patch('api.routes.reports_routes.report_generator') as mock_generator, \
         patch('api.routes.reports_routes.tax_calculator') as mock_calculator:
        ...
        response = client.post('/reports/quarterly-tax', json={
            'user_id': 1,
            'year': 2023,
            'quarter': 1
        })
...
def test_generate_custom_report(client):
    ...
    with patch('api.routes.reports_routes.report_generator') as mock_generator:
        ...
        response = client.post('/custom-report', json={
            'user_id': 1,
...
def test_analyze_tax_savings(client):
    ...
    with patch('api.routes.reports_routes.report_generator') as mock_generator:
        mock_generator.analyze_tax_savings.return_value = {
            ...
            'recommendations': [
                'Consider tracking mileage',
                'Keep meal receipts'
            ]
        }
        
        response = client.post('/tax-savings', json={
            'user_id': 1,
            'year': 2023
        })
