...
def test_validate_platform_data_empty_fields():
    data = {"platform": "", "user_id": "", "access_token": ""}
    errors = validate_platform_data(data)
    assert len(errors) > 0


def test_validate_expense_valid():
    data = {"amount": 100.50, "description": "Test expense"}
