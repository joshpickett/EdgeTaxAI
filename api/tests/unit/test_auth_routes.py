...
def test_signup_invalid_email(client):
    ...
    data = {"email": "invalid-email", "phone_number": "+1234567890"}
...
def test_verify_otp_success(client, test_db):
    ...
    response = client.post(
        "/api/auth/verify-otp", data=json.dumps(data), content_type="application/json"
    )
...
def test_biometric_register_success(client):
    ...
    data = {"user_id": 1, "biometric_data": "test_biometric_data"}

    with patch(
        "api.routes.auth_routes.biometric_auth.register_biometric"
    ) as mock_register:
