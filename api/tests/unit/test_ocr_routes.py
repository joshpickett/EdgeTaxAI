...
def test_process_receipt(client):
    ...
    with patch("api.routes.ocr_routes.vision") as mock_vision, patch(
        "api.routes.ocr_routes.expense_integration"
    ) as mock_expense:

        # Mock Vision API response
        mock_text = MagicMock()
        mock_text.description = "Test Receipt\nAmount: $50.00"
        mock_vision.ImageAnnotatorClient().text_detection.return_value.text_annotations = [
            mock_text
        ]
...
def test_analyze_receipt(client):
    ...
    with patch("api.routes.ocr_routes.vision") as mock_vision, patch(
        "api.routes.ocr_routes.extract_receipt_data"
    ) as mock_extract:
        ...
        response = client.post(
            "/analyze-receipt", content_type="multipart/form-data", data=data
        )
...
def test_extract_expense(client):
    ...
    with patch("api.routes.ocr_routes.extract_receipt_data") as mock_extract:
        mock_extract.return_value = {"amount": 50.00, "category": "office_supplies"}

        response = client.post(
            "/extract-expense", json={"text": "Office supplies receipt $50.00"}
        )
