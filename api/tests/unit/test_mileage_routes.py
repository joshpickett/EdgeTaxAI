...
def test_calculate_mileage(client):
    ...
    with patch('api.routes.mileage_routes.fetch_google_directions') as mock_directions:
        ...
        response = client.post('/mileage', json={
            'start': '123 Start St',
            'end': '456 End Ave',
...
def test_add_mileage_record(client):
    ...
    with patch('api.routes.mileage_routes.get_db_connection') as mock_db, \
         patch('api.routes.mileage_routes.fetch_google_directions') as mock_directions:
        ...
        response = client.post('/mileage/add', json={
            'user_id': 1,
...
def test_bulk_mileage_upload(client):
    ...
    with patch('api.routes.mileage_routes.fetch_google_directions') as mock_directions:
        ...
        response = client.post('/mileage/bulk', json={
            'records': [
                {
                    'start': '123 Start St',
                    'end': '456 End Ave',
