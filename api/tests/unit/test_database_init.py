...
def test_database_initialization(test_database_path):
    ...
    required_tables = {
        'users', 'expenses', 'mileage', 'reminders', 
        'bank_accounts', 'bank_transactions', 
        'category_learning', 'category_patterns'
    }
...
def test_table_schemas(test_database_path):
    ...
    assert {'id', 'full_name', 'email', 'phone_number', 'password', 
            'otp_code', 'otp_expiry', 'is_verified', 'tax_strategy'
           }.issubset(user_columns)
    
    # Test expenses table schema
    cursor.execute("PRAGMA table_info(expenses);")
    expense_columns = {row[1] for row in cursor.fetchall()}
    assert {'id', 'user_id', 'description', 'amount', 'category', 
            'confidence_score', 'tax_context', 'learning_feedback', 
            'date', 'receipt'
...
def test_foreign_key_constraints(test_database_path):
    ...
    # Test foreign key constraints
    tables_with_foreign_key = ['expenses', 'mileage', 'reminders', 
                      'bank_accounts', 'bank_transactions']
    ...
    for table in tables_with_foreign_key:
        ...
        assert any(foreign_key[2] == 'users' for foreign_key in foreign_keys)  # References users table
...
def test_default_values(test_database_path):
    ...
    cursor.execute("SELECT is_verified, tax_strategy FROM users WHERE email=?", 
                  ("test@example.com",))
    row = cursor.fetchone()
