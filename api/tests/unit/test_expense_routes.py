...
def test_get_expense_summary(client):
    ...
    with patch("api.routes.expense_routes.report_generator") as mock_generator:
        ...
        mock_generator.generate_expense_summary.return_value = {
            "total": 1000.00,
            "categories": {"gas": 300.00, "maintenance": 700.00},
        }
...
def test_create_expense(client):
    ...
    with patch("api.routes.expense_routes.get_db_connection") as mock_db, patch(
        "api.routes.expense_routes.categorize_expense"
    ) as mock_categorize:
        ...
        mock_categorize.return_value = {"category": "vehicle", "confidence_score": 0.95}
...
def test_create_platform_expense(client):
    ...
    with patch("api.routes.expense_routes.get_db_connection") as mock_db, patch(
        "api.routes.expense_routes.analyze_tax_context"
    ) as mock_tax:
        ...
        # Mock tax analysis
        mock_tax.return_value = {"deductible": True, "category": "vehicle_expense"}
