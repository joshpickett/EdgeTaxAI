...
from ...middleware.auth_middleware import (
    token_required, generate_token, needs_refresh,
    refresh_token, rate_limit, APIError, SessionManager
)
...
def test_refresh_token(mock_jwt):
    ...
    old_token = "old_token"
    mock_jwt['decode'].return_value = {'user_id': 1, 'exp': (datetime.utcnow() + timedelta(minutes=4)).timestamp()}
