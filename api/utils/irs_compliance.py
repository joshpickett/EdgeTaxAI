...
class IRSCompliance:
    ...
    def verify_compliance(self, expense: Dict[str, Any]) -> Dict[str, Any]:
        ...
        required_docs = self.compliance_rules["documentation_required"].get(
            category, []
        )

        # Check required documentation
        missing_docs = [doc for doc in required_docs if not expense.get(doc)]

        # Calculate compliance score
        compliance_score = (
            1.0 - (len(missing_docs) / len(required_docs)) if required_docs else 1.0
        )
