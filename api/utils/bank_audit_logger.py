...
class BankAuditLogger:
    ...
    def log_connection_attempt(
        self, user_id: int, bank_name: str, status: str, error: Optional[str] = None
    ) -> None:
        """Log bank connection attempts"""
        self._log_event(
            user_id=user_id,
            event_type="connection_attempt",
            details={"bank_name": bank_name, "status": status, "error": error},
        )

    def log_transaction_sync(
        self, user_id: int, bank_account_id: int, transaction_count: int, status: str
    ) -> None:
