...
class EstimatedTaxTracker:
    ...
    async def calculate_estimated_payment(
        self, income_data: Dict[str, Any], quarter: int
    ) -> Dict[str, Any]:
        ...
        return {
            "safe_harbor_amount": self._calculate_safe_harbor(
                estimated_tax, income_data.get("prior_year_tax", 0)
            ),
            "minimum_required": self._calculate_minimum_required(
                estimated_tax, income_data.get("prior_year_tax", 0), quarter
            ),
            "quarter": quarter,
    ...
    def get_payment_history(self, user_id: int, tax_year: int) -> List[Dict[str, Any]]:
        """Get estimated tax payment history"""
        payments = (
            self.db.query(TaxPayments)
            .filter(
    ...
    def get_payment_schedule(self, user_id: int, tax_year: int) -> List[Dict[str, Any]]:
    ...
    def _calculate_safe_harbor(
        self, current_year_tax: Decimal, prior_year_tax: Decimal
    ) -> float:
        """Calculate safe harbor amount"""
        return float(
            min(current_year_tax * self.safe_harbor_percentage, prior_year_tax)
        )

    def _calculate_minimum_required(
        self, estimated_tax: Decimal, prior_year_tax: Decimal, quarter: int
    ) -> float:
