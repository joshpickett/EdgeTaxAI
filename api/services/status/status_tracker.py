...
class StatusTracker:
    ...
    def __init__(self):
        ...
        self.workflow_states = {
            ...
            "REJECTED": {"next_states": ["UPLOADED"], "required_checks": []},
            "FAILED": {"next_states": ["UPLOADED"], "required_checks": []},
            "ARCHIVED": {"next_states": [], "required_checks": []},
        }

    async def track_document_status(
        self, document_id: int, new_status: str, metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            check_results = await self._perform_required_checks(document, new_status)

            if not check_results["passed"]:
                return {"success": False, "errors": check_results["errors"]}

            # Update document status
            await self._update_document_status(document_id, new_status, metadata)

            # Log status change
            await self.audit_logger.log_status_change(
                document_id, current_status, new_status, metadata
            )

            # Check expiration status and send notifications if needed
            expiration_info = await self._check_expiration_status(document)
            if (
                expiration_info["is_expired"]
                or expiration_info["days_until_expiration"] < 30
            ):
    ...
    async def get_document_checklist(self, document_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            checklist = {
                ...
                "required_checks": self.workflow_states[current_state][
                    "required_checks"
                ],
            }
    ...
    async def monitor_expiration(self, document_id: int) -> Dict[str, Any]:
    ...
    async def _perform_check(self, document: Document, check: str) -> Dict[str, Any]:
        ...
        if check not in check_functions:
            return {"passed": False, "error": f"Unknown check type: {check}"}
    ...
    async def _send_expiration_notification(
        self, document: Document, expiration_info: Dict[str, Any]
    ) -> None:
