...
class DeductionOptimizationService:
    ...
    async def optimize_deductions(self, user_id: int, tax_year: int) -> Dict[str, Any]:
        ...
        try:
            ...
            foreign_housing = await self._get_foreign_housing_expenses(
                user_id, tax_year
            )
            foreign_tax_paid = await self._get_foreign_tax_paid(user_id, tax_year)

            optimizations = {
                "foreign_housing_deduction": self._optimize_foreign_housing(
                    foreign_housing
                ),
                "foreign_tax_credit": self._optimize_foreign_tax_credit(
                    foreign_tax_paid
                ),
    ...
    def _calculate_standard_mileage(
        self, mileage_records: List[Dict[str, Any]]
    ) -> Decimal:
        """Calculate standard mileage deduction"""
        total_miles = sum(record["distance"] for record in mileage_records)
        return (
            Decimal(str(total_miles))
            * self.deduction_categories["vehicle"]["standard_rate"]
        )

    def _calculate_actual_vehicle_expenses(
    ...
    def _calculate_actual_home_office(self, data: Dict[str, Any]) -> Decimal:
        ...
        total_home_expenses = sum(
            Decimal(str(amount)) for amount in data["home_expenses"].values()
        )
        business_percentage = Decimal(str(data["office_square_footage"])) / Decimal(
            str(data["total_square_footage"])
        )
        return total_home_expenses * business_percentage

    def _optimize_foreign_housing(
        self, housing_expenses: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Optimize foreign housing deduction"""
        try:
            base_amount = self.deduction_categories["foreign_housing"]["thresholds"][
                "base_amount"
            ]
            max_amount = self.deduction_categories["foreign_housing"]["thresholds"][
                "maximum_amount"
            ]

            qualified_expenses = sum(
                Decimal(str(amount)) for amount in housing_expenses.values()
            )
    ...
    def _recommend_home_office_method(
        self, simplified: Decimal, actual: Decimal
    ) -> Dict[str, Any]:
            ...
            recommendations.append(
                {
                    ...
                    "documentation_needed": self.deduction_categories[category][
    ...
    def _get_action_items(
        self, category: str, opportunities: List[Dict[str, Any]]
    ) -> List[str]:
