...
class ScheduleManagementService:
                ...
                for required_form in deps.get("required_forms", []):
                    if required_form not in schedules:
                        validation_results["missing_dependencies"].append(
                            {"schedule": schedule, "missing": required_form}
                        )
    ...
    async def validate_cross_schedule_consistency(
        ...
        try:
            validation_results = await self.cross_calculator.validate_consistency(
                schedules
            )
            return validation_results
        except Exception as e:
    ...
    def _needs_schedule_c(self, tax_data: Dict[str, Any]) -> bool:
        ...
        return (
            income_sources.get("business_income", 0) > 0
            or income_sources.get("self_employment", 0) > 0
        )

    def _needs_schedule_se(self, tax_data: Dict[str, Any]) -> bool:
        """Determine if Schedule SE is needed"""
        self_employment_income = tax_data.get("income_sources", {}).get(
            "self_employment_income", 0
        )
        return (
            self_employment_income
            > self.schedule_dependencies["SCHEDULE_SE"]["threshold"]
        )
    ...
    def _needs_schedule_e(self, tax_data: Dict[str, Any]) -> bool:
        ...
        income_sources = tax_data.get("income_sources", {})
        return (
            income_sources.get("rental_income", 0) > 0
            or income_sources.get("partnership_income", 0) > 0
        )

    async def generate_schedule(
        self, schedule_type: str, tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation_result["is_valid"]:
                raise ValueError(
                    f"Invalid schedule data: {validation_result['errors']}"
                )
    ...
    def _validate_schedule_requirements(
        self, schedule_type: str, tax_data: Dict[str, Any]
    ) -> bool:
        """Validate that all required fields are present"""
        required_fields = self.schedule_dependencies.get(schedule_type, {}).get(
            "required_fields", []
        )
    ...
    async def _generate_schedule_data(
        self, schedule_type: str, tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def validate_schedule_completeness(
        self, schedule_type: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
