...
def validate_foreign_address(address: Dict[str, Any]) -> List[str]:
    ...
    if address.get("postal_code") and not BusinessRules._validate_postal_code(
        address["postal_code"], address.get("country")
    ):
...
def validate_currency(
    amount: Decimal, currency_code: str, exchange_rate: Dict[str, Any]
) -> List[str]:
...
class ValidationRules:
    ...
    async def validate_1099_nec(
        self, data: Dict[str, Any], document_metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        compensation = Decimal(
            str(data.get("payments", {}).get("nonemployee_compensation", 0))
        )
        ...
        if state_info:
            validation_result["errors"].extend(
                BusinessRules.validate_state_withholding(state_info)
            )
        ...
        required_fields = [
            ...
            (
                "payments.nonemployee_compensation",
                "Nonemployee compensation is required",
            ),
        ]

        validation_result["errors"].extend(
            ValidationRules._check_required_fields(data, required_fields)
        )
    ...
    def validate_1099_k(self, data: Dict[str, Any]) -> List[str]:
        ...
        transaction_count = int(
            data.get("transactions", {}).get("card_transactions", 0)
        )
        if transaction_count < BusinessRules.K_TRANSACTION_THRESHOLD:
            errors.append(
                f"Must have {BusinessRules.K_TRANSACTION_THRESHOLD} or more transactions"
            )

        # Validate monthly amounts
        monthly_amounts = {
            f"month_{i}": Decimal(
                str(data.get("transactions", {}).get(f"month_{i}", 0))
            )
            for i in range(1, 13)
        }
    ...
    def validate_1040ez(self, data: Dict[str, Any]) -> List[str]:
        ...
        if total_income > BusinessRules.EZ_INCOME_THRESHOLD:
            errors.append(
                f"Income must be less than ${BusinessRules.EZ_INCOME_THRESHOLD} to use Form 1040-EZ"
            )
    ...
    def validate_schedule_d(self, data: Dict[str, Any]) -> List[str]:
        ...
        for transaction in short_term:
            errors.extend(
                ValidationRules._validate_transaction(transaction, "short-term")
            )
        ...
        for transaction in long_term:
            errors.extend(
                ValidationRules._validate_transaction(transaction, "long-term")
            )

        return errors

    def _validate_transaction(
        transaction: Dict[str, Any], trans_type: str
    ) -> List[str]:
        ...
        required_fields = [
            ...
            (
                "date_acquired",
                f"Acquisition date required for {trans_type} transaction",
            ),
        ...
        errors.extend(
            ValidationRules._check_required_fields(transaction, required_fields)
        )
        return errors
