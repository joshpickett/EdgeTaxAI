...
class SchemaManager:
    ...
    def detect_version(self, xml_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            version = root.get("version") or self._extract_version_from_namespace(
                namespace
            )
    ...
    def validate_schema(
        self, xml_content: str, form_type: str, version: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def _validate_with_backward_compatibility(
        self, xml_content: str, form_type: str, version: str
    ) -> Dict[str, Any]:
        """Validate XML with backward compatibility"""
        try:
            # Load historical schema version
            historical_schema_file = (
                f"{self.schema_path}/{form_type.lower()}_{version}.xsd"
            )
    ...
    def _detect_form_type(self, root: etree.Element) -> str:
        ...
        for form_type, indicators in form_indicators.items():
            if any(
                root.find(f".//{indicator}") is not None for indicator in indicators
            ):
                return form_type
