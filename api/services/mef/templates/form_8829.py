...
class Form8829Template(BaseMeFTemplate):
    ...
    def _add_home_office_info(self, parent: ElementTree.Element, data: Dict[str, Any]) -> None:
        """Add home office information"""
        ...
        percentage.text = str(self._calculate_percentage(
            data.get('office_area', 0),
            data.get('total_area', 1)
        ))
    ...
    def _add_expense_category(self, parent: ElementTree.Element, expenses: Dict[str, Any]) -> None:
        """Add expense category details"""
        categories = [
            'repairs', 'utilities', 'insurance', 'maintenance',
            'mortgage_interest', 'property_taxes', 'other'
        ]
