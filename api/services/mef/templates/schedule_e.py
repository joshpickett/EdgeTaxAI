...
class ScheduleETemplate(BaseScheduleTemplate):
    ...
    def _calculate_totals(self, properties: List[Dict[str, Any]]) -> Dict[str, Decimal]:
        ...
        for property_data in properties:
            ...
            total_income = Decimal(str(income.get("rents", 0))) + Decimal(
                str(income.get("other", 0))
            )
    ...
    def _add_totals(
        self, parent: ElementTree.Element, totals: Dict[str, Decimal]
    ) -> None:
        """Add totals section"""
