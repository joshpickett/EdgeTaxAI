...
class DocumentRequirementsMapper:
    ...
    def get_required_documents(
        ...
        try:
            ...
            if answers.get('has_self_employment'):
                self._add_self_employment_requirements(required_documents, optional_documents)
            ...
            if answers.get('has_business_expenses'):
                self._add_business_expense_requirements(required_documents, optional_documents)
    ...
    def _add_self_employment_requirements(self, required: List[Dict], optional: List[Dict]) -> None:
        """Add self-employment specific requirements"""
        self_employment_documents = self.schedule_requirements['SCHEDULE_C']
        required.extend([
            document for document in self_employment_documents['required']
            if document not in required
        ])
        optional.extend([
            document for document in self_employment_documents.get('optional', [])
            if document not in optional
        ])
    
    def _add_rental_requirements(self, required: List[Dict], optional: List[Dict]) -> None:
        """Add rental income specific requirements"""
        rental_documents = self.schedule_requirements['SCHEDULE_E']
        required.extend([
            document for document in rental_documents['required']
            if document not in required
        ])
        optional.extend([
            document for document in rental_documents.get('optional', [])
            if document not in optional
        ])
    
    def _add_business_expense_requirements(self, required: List[Dict], optional: List[Dict]) -> None:
