...
class FormProgressManager:
    ...
    async def save_progress(
        self, user_id: int, form_type: str, progress_data: Dict[str, Any], db: Session
    ) -> None:
        """Save form progress"""
        try:
    ...
    async def get_progress(
        self, user_id: int, form_type: str, db: Session
    ) -> Optional[Dict[str, Any]]:
        """Get saved progress"""
        try:
            progress = (
                db.query(FormProgress)
                .filter(
                    FormProgress.user_id == user_id, FormProgress.form_type == form_type
                )
                .first()
            )
    ...
    async def clear_progress(self, user_id: int, form_type: str, db: Session) -> None:
    ...
    async def get_all_progress(self, user_id: int, db: Session) -> Dict[str, Any]:
        """Get all saved progress for user"""
        try:
            progress_records = (
                db.query(FormProgress).filter(FormProgress.user_id == user_id).all()
            )
