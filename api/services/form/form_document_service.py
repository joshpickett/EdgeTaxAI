...
class FormDocumentService:
    ...
    async def get_required_documents(
        ...
        try:
            ...
            # Filter by form type
            form_requirements = self._filter_requirements(
                checklist, form_type
            )
    ...
    def _filter_requirements(
        ...
        return {
            'required': [
                doc for doc in checklist['required']
                if doc['type'] in required_types
            ],
            'optional': [
                doc for doc in checklist['optional']
                if doc['type'] in required_types
            ]
        }
    ...
    async def _validate_document_requirements(
        ...
        if not self._is_valid_document_type(document.type, form_type):
            ...
            validation_result['errors'].append(
                'Invalid document type for form'
            )
            
        # Validate document content
        content_validation = await self._validate_document_content(
            document
        )
    ...
    def _is_valid_document_type(
        ...
        valid_types = {
            ...
            '1099_K': [
                ...
                DocumentType.PLATFORM_STATEMENT
            ]
        }
    ...
    async def _get_submitted_documents(
        self,
        user_id: int
    ) -> List[str]:
