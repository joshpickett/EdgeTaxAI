...
class FormScheduleService:
    ...
    async def generate_summary(self, user_id: int, tax_year: int) -> Dict[str, Any]:
        ...
        try:
            ...
            required_schedules = (
                await self.schedule_manager.determine_required_schedules(
                    user_id, tax_year
                )
            )
            ...
            for schedule in required_schedules:
                schedule_data[schedule] = await self.schedule_manager.generate_schedule(
                    schedule, {"user_id": user_id, "tax_year": tax_year}
                )
    ...
    async def validate_schedule(
        ...
        try:
            validation_result = (
                await self.schedule_manager.validate_schedule_completeness(
                    schedule_type, data
                )
            )
    ...
    async def get_schedule_dependencies(self, schedule_type: str) -> Dict[str, Any]:
