...
class ScheduleSummaryService:
    ...
    def _summarize_schedule_c(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        expenses = sum(
            Decimal(str(amount))
            for amount in data.get('expenses', {}).values()
        )
        ...
        return {
            ...
            'expense_categories': self._summarize_expense_categories(data.get('expenses', {}))
        }
    ...
    def _summarize_schedule_e(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for prop in properties:
            total_income += sum(Decimal(str(amount)) for amount in prop.get('income', {}).values())
            total_expenses += sum(Decimal(str(amount)) for amount in prop.get('expenses', {}).values())
    ...
    def _summarize_property(self, property_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        income = sum(Decimal(str(amount)) for amount in property_data.get('income', {}).values())
        expenses = sum(Decimal(str(amount)) for amount in property_data.get('expenses', {}).values())
