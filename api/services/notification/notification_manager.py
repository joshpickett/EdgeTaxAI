...
class NotificationManager:
    ...
    async def send_notification(
        ...
        try:
            ...
            notification = await self._create_notification(
                user_id, notification_type, notification_config, data
            )

            # Send through appropriate channels
            for channel in notification_config["channels"]:
                await self._send_through_channel(
                    notification, channel, notification_config
                )
    ...
    async def _send_through_channel(
        self, notification: Notification, channel: str, config: Dict[str, Any]
    ) -> None:
    ...
    async def _send_email_notification(
        self, notification: Notification, config: Dict[str, Any]
    ) -> None:
        """Send email notification"""
        template = config["template"]
        await self.email_service.send_templated_email(
            notification.user_id, template, notification.data
        )
