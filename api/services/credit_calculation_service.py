...
class CreditCalculationService:
    ...
    def _calculate_child_tax_credit(self, tax_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            qualifying_children = tax_data.get('dependents', {}).get('qualifying_children', 0)
    ...
    def _calculate_earned_income_credit(self, tax_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if earned_income > income_limit:
                return {'amount': Decimal('0'), 'qualifying_children': qualifying_children}
            
            credit_percentage = self._get_eic_percentage(qualifying_children)
            calculated_credit = (earned_income * credit_percentage).quantize(Decimal('0.01'))
    ...
    def _calculate_education_credits(self, tax_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            education_expenses = Decimal(str(
                tax_data.get('education', {}).get('qualified_expenses', 0)
            ))
            ...
            american_opportunity_credit_amount = min(education_expenses, Decimal('4000')) * Decimal('0.25')
            
            # Calculate Lifetime Learning Credit
            lifetime_learning_credit_amount = min(education_expenses, Decimal('10000')) * Decimal('0.2')
    ...
    def _generate_qualification_summary(
        ...
        return {
            'qualified_credits': [
                name for name, data in credits.items()
                if data.get('amount', 0) > 0
            ],
            'total_qualified': sum(
                1 for data in credits.values()
                if data.get('amount', 0) > 0
            ),
            'requirements_met': {
    ...
    def _check_requirements(
        ...
        requirements = {
            'child_tax_credit': {
                'has_qualifying_children': credit_data.get('qualifying_children', 0) > 0,
