...
def process_file(source_path: str, test_path: str) -> None:
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a test generation assistant. Generate comprehensive tests for the given code.",
            },
        ...
        response = openai_handler.generate_completion(
            messages=messages, model=Config.MODEL_NAME, temperature=0.7, max_tokens=2000
        )
...
def process_source_directories() -> None:
    ...
    logging.info(
        f"Processing files in batches of {batch_size} with {max_workers} workers"
    )
